...
def generate_chromo_df():
    ...
    csv_path = os.path.join(current_dir, "chromo_db.csv")
    chromo_db = pd.read_csv(csv_path)
...
def draw_chromo_graph(graph, overlap_df, solvent, overlap_threshold):
    ...
    plt.title(
        f"Chromophore Graph\nSolvent: {solvent}, % Overlap Thresh: {overlap_threshold}%"
    )


if __name__ == "__main__":
    print("generating chromo df...")
    generate_chromo_df()
    chromo_db = pd.read_pickle("chromo_db_filtered.pkl")
    print("generating percent overlap df...")
    generate_overlap_df(chromo_db)
    overlap_df = pd.read_pickle("percent_overlap_df.pkl")
    print("pickle files saved.")

    overlap_df_cleaned = overlap_df.dropna(how="any")
    solvent = "acetonitrile"
    overlap_threshold = 0.8

    print("Generating chromo overlap graph...")
    G = create_chromo_graph(overlap_df_cleaned, solvent, overlap_threshold)

    draw_chromo_graph(G, overlap_df_cleaned, solvent, overlap_threshold)
