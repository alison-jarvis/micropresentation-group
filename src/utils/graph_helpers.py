...
def create_chromo_graph(
    ...
    overlap_df = overlap_df[
        (overlap_df["Percent Overlap"].fillna(0) >= overlap_threshold)
        & (overlap_df["Solvent"] == solvent)
    ]
    ...
    G.add_nodes_from(
        zip(
            list(solvent_chromo_df["Chromophore"]),
            solvent_chromo_df.to_dict(orient="records"),
        )
    )
    return G


def get_peak_distance_heuristic(
    solvent: str, chromo_df: pd.DataFrame, heuristic_scaling: float
) -> Callable:
    """
    ...
    def heuristic(chromo_b, chromo_a):
        ...
        return heuristic_scaling * abs(emission_wl - absorbtion_wl)
...
def print_path(path: list[str], graph: nx.Graph) -> None:
    ...
    for node in path:
        ...
        print(
            f"{disp_node}: Absorption - {absorbtion_wl:.2f} nm -- Emission - {emission_wl:.2f} nm"
        )
